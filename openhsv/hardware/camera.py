...
_EXAMPLE_VIDEO_PATH = Path(__file__) / Path(r"..\..\examples\thethroat.mp4")
...
class DummyCamera(Camera):
    def __init__(self, video_path: Path=_EXAMPLE_VIDEO_PATH, is_color=True, verbose=True):
        self.verbose = verbose 
        self.is_color = is_color
        self._idle = True
        self.i = 0
        # Needed for saving metadata
        self.videoFrames = io.mimread(str(video_path), memtest=False)
        self.frames_to_record = self.videoFrames
        self.frames_before_trigger = self.videoFrames

    def openCamera(self):
        logging.info("Dummy camera successfully opened!")
        return True 

    def configCam(self):
        logging.info("Dummy camera successfully configured!")
        return True

    def setSettings(self, *args, **kwargs):
        logging.info("Dummy camera settings were set.")
        return True 
    ...
    def startGrab(self):
        logging.info("Start grabbing dummy camera")
        self.i = 0
        self._idle = False
        return True 

    def stopGrab(self):
        logging.info("Stop grabbing dummy camera")
        self._idle = True
        return True

    def live(self):
        self.i += 1
        return self.videoFrames[self.i % len(self.videoFrames)]

    def getMemoryFrame(self, frame_index, by_trigger=True):
        return self.videoFrames[frame_index % len(self.videoFrames)]
...
class IdtCamera(Camera):
    ...
    def stopGrab(self):
        ...
        logging.info("Recorded frames : ", recorded_frames)
